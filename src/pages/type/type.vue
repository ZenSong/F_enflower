                ...
                <ul>  
                    <li> 
                        <div  >  
                            ...
                            <h3 style="text-align: center; color: rgb(249, 111, 145); font-weight: bold; margin: 0.5rem 0;font-size: 1.1rem;">{{flowerData.flowerName}}</h3> 
                            <div class="innerHtml"> 
            ...
            <div class="content_list"  id="content_list"  >
                <ul>
                    ...
                    <li class="video" >
                        ...
                        <!-- :poster="flowerData.videoImgAddr" -->
                        <video v-if="myVideUrl != ''"
                            id="myVideo"
                            class="video-js    vjs-big-play-centered    vjs-has-started"
                            controls 
                            webkit-playsinline="true"
                            playsinline="true" 
                            controlslist="nodownload nofullscreen noremoteplayback" 
                            >
                            <source  :src="myVideUrl"  type="video/mp4"><source> 
    ...
    methods : {
        ...
        getDetailData() { 
            ...
            .then(res => {  
                ...
                // 视频
                if(this.flowerType=="Video" || this.flowerType=="video"){
                    this.myVideUrl = this.flowerData.videoAddr
                    this.count++
                    this.$nextTick(() => {
        ...
        myFull() {
            let that = this
            window.addEventListener("onorientationchange" in window ? "orientationchange" : "resize", this.stateLed, false); 
            var player = that.videojs(document.getElementById('myVideo'), {
                ...
                let father = document.querySelector("#myVideo")
                let isfull = document.fullscreenElement || document.mozFullScreenElement || document.webkitFullscreenElement;  
                if(isfull) {   
                    that.myFullScreen = true

                    that.stateLed()
                }
                else {
                    that.myFullScreen = false
                    let videoAdv = document.querySelector(".vidoe-adv")   
                    if(videoAdv) { 
                        father.setAttribute("style","height:auto !important;margin-top: 0 !important")   
                        let vjsTech = document.querySelector(".vjs-tech") 
                        vjsTech.setAttribute("style","margin-top: 0 !important;height: 100% !important")   
                        let vjsTextTrackDisplay = document.querySelector(".vjs-text-track-display") 
                        vjsTextTrackDisplay.setAttribute("style","margin-top: 0 !important")
                        father.removeChild(videoAdv) 
                    } 
                }
                }) 
            }) 
        ...
        stateLed() { 
            if (window.screen.orientation.angle == 180 || window.screen.orientation.angle == 0) {
                ...
                if(this.myFullScreen) {
                    let father = document.querySelector("#myVideo")
                    if(father) {
                        if(Array.from(father.querySelectorAll(".vidoe-adv")).length == 0) {  
                            father.setAttribute("style","height:calc(100% - 100px) !important;margin-top: 100px !important")  
                            let vjsTech = document.querySelector(".vjs-tech") 
                            vjsTech.setAttribute("style","margin-top: 100px !important;height:calc(100% - 100px) !important")   
                            let vjsTextTrackDisplay = document.querySelector(".vjs-text-track-display") 
                            vjsTextTrackDisplay.setAttribute("style","margin-top: 100px !important")
                            let advDiv = document.createElement("div") 
                            advDiv.setAttribute('class','vidoe-adv googleListVideo')
                            advDiv.innerHTML =` <ins class="adsbygoogle"
                                    style="display:inline-block;width:100%; height:100%;"
                                    data-ad-client="ca-pub-7936490518220507"
                                    data-ad-slot="8268485993"></ins>`  
                            father.appendChild(advDiv)

                            var myVar=setTimeout(() => {  
                                var oDom=Array.from(document.querySelectorAll(".googleListVideo")) 
                                for(var i=0;i<oDom.length;i++ ){ 
                                    var ss = oDom[i].querySelectorAll(".mygoogleListVideo"); 
                                    if(ss.length > 0 && ss[0].innerHTML == '(adsbygoogle = window.adsbygoogle || []).push({});') {
                                        ...
                                        script.type='text/javascript';
                                        var nHTML='(adsbygoogle = window.adsbygoogle || []).push({});'
                                        script.setAttribute('class','mygoogleListVideo');
                                        script.innerHTML=nHTML
                                        oDom[i].appendChild(script);
                                    } 
                                }
            ...
            if (window.screen.orientation.angle == 90 || window.screen.orientation.angle == -90) {
                ...
                if(this.myFullScreen) {
                    let father = document.querySelector("#myVideo")
                    let vidoeAdv = document.querySelector(".vidoe-adv")
                    if(father && vidoeAdv) {
                        father.setAttribute("style","height:auto !important;margin-top: 0 !important")   
                        let vjsTech = document.querySelector(".vjs-tech") 
                        vjsTech.setAttribute("style","margin-top: 0 !important;height: 100% !important")   
